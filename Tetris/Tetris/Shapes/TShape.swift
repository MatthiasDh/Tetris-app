...
class TShape : Block {
    ...
    override func rotate() {
        ...
        if rotation == 1 {
            sprite4?.position = (sprite1?.position)!
            sprite1?.position = (sprite2?.position)!
            sprite2?.position.y = (sprite2?.position.y)! - TileHeight;
            sprite2?.position.x = (sprite2?.position.x)! - TileHeight;
            self.row = self.row - 1
        } else if rotation == 2 {
            sprite4?.position = (sprite1?.position)!
            sprite1?.position = (sprite2?.position)!
    ...
    override func addSprites(blocksLayer: SKNode) {
        ...
        if vertical {
            ...
            sprite2?.position = pointFor(column: column+1, row: row-1)
            sprite2?.name = "block"
